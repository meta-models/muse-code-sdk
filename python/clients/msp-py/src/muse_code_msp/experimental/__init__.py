...
"""Generated MSP wire types, EXPERIMENTAL surface. Explicit opt-in import only; nothing here carries a stability promise (spec 638 FR-638-004)."""

from __future__ import annotations

from typing import Any, Final, Literal, TypedDict

SCHEMA_FINGERPRINT: Final[str] = "sha256:3ebfa82522616e8bef05585ee0c9c84263956b95fa991d45f5b05ed93e3ea9a3"
...
AccountLoginOutcome = str  # open enum (SS1.5.4): unknown values stay representable
ACCOUNT_LOGIN_OUTCOME_KNOWN_VALUES: Final[tuple[str, ...]] = ("granted", "denied", "expired", "cancelled", "failed",)
...
AccountLoginType = Literal["deviceCode", "apiKey"]
...
AccountStateKind = str  # open enum (SS1.5.4): unknown values stay representable
ACCOUNT_STATE_KIND_KNOWN_VALUES: Final[tuple[str, ...]] = ("loggedOut", "envKey", "apiKey", "accountLogin",)
...
class AccountLoginCancelResult(TypedDict):
    """`account/loginCancel` result: exactly `{cancelled}` — `true` iff a pending flow existed and was cancelled (spec 28091 FR-29535-5)."""

    cancelled: bool


class _AccountLoginCompletedParamsBase(TypedDict):
    outcome: AccountLoginOutcome


class AccountLoginCompletedParams(_AccountLoginCompletedParamsBase, total=False):
...
class _AccountLoginStartParamsBase(TypedDict):
    type: AccountLoginType


class AccountLoginStartParams(_AccountLoginStartParamsBase, total=False):
...
class AccountLoginStartResult(TypedDict, total=False):
...
class _AccountStateBase(TypedDict):
...
METHODS: Final[dict[str, MethodSpec]] = {
    "account/loginCancel": {"description": "Cancels the pending login flow, abandoning the device code to server-side expiry; cancelled is false when nothing was pending (spec 28091 FR-29535-5).", "params": None, "result": "AccountLoginCancelResult"},
    "account/loginStart": {"description": "Starts a host-owned login flow: the RFC 8628 device-code flow (result carries the verification URL and user code for native rendering) or the apiKey type (stores the carried key; completes synchronously). Supersedes a pending flow (spec 28091 FR-29535-1/INV-29535-2).", "params": "AccountLoginStartParams", "result": "AccountLoginStartResult"},
    "account/logout": {"description": "Clears the stored credential (with the best-effort pre-clear session revoke) and answers the fresh projected state (spec 28091 FR-29535-6).", "params": None, "result": "AccountState"},
    "account/read": {"description": "Reads the host's projected credential state: which credential lane is in effect, an optional display label, and whether the deployment needs a credential at all (spec 28091 FR-001).", "params": None, "result": "AccountState"},
...
NOTIFICATIONS: Final[dict[str, NotificationSpec]] = {
    "account/changed": {"description": "The host's projected credential state changed — including out-of-band changes such as a terminal `muse logout` — with the same payload shape as account/read (spec 28091 FR-002).", "params": "AccountState"},
    "account/loginCompleted": {"description": "A login flow reached a terminal outcome (granted, denied, expired, cancelled, or failed); a granted flow's account/changed follows (spec 28091 FR-29535-4; ADR 28091 D3). Never carries the device-code artifacts or any key material.", "params": "AccountLoginCompletedParams"},
